...
Close every item marked `planned` in `BORLAND-V1-COMPATIBILITY.md`, especially the logarithmic least-squares helper, remaining FFT helpers and demonstration/sample applications. Complete or explicitly supersede the remaining items marked `partial`, notably the five-term Fourier least-squares helper and the compact real-spectrum FFT convenience API.
