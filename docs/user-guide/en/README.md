...
The committed V1.0 PDFs are generated from Markdown through `../LaTeX/`. The LaTeX directory supplies typesetting, front/back matter and chapter ordering; it is not a second independent copy of the technical prose. For stable version 1.0 the LaTeX order, chapters 12/13 and release metadata were synchronized, both language editions were regenerated, structurally validated and visually inspected. Future editorial changes continue to start in Markdown and are synchronized into the PDF editions at release-candidate/release boundaries.
