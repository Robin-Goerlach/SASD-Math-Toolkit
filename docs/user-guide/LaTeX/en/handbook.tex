...
\newcommand{\HandbookPdfTitle}{SASD Math Toolkit - CSharp/.NET Handbook Version 1.0}
\newcommand{\HandbookFooter}{Handbook Version 1.0 - English}
\newcommand{\HandbookSubtitle}{Numerical Methods - C\# / .NET}
\newcommand{\HandbookVersionText}{User and Developer Handbook - Version 1.0}
...
\begin{sasdnote}{Handbook Version 1.0}
This document is the English handbook release 1.0 for the C\#/.NET implementation of SASD Math Toolkit. During the build, chapter content is generated from the maintained Markdown handbook under \texttt{docs/user-guide/en/}. Markdown therefore remains the editorial source of truth while LaTeX provides typography, navigation and PDF output.
\end{sasdnote}
...
SASD Math Toolkit carries the functional core of a classical numerical-methods toolbox into a modern, independently developed C\#/.NET library. The historical manual is used as orientation for problem classes and handbook structure, not as a source-text or source-code template.

The historical edition consistently organized numerical routines around the problem, description, inputs, outputs, call syntax, comments and worked examples. This edition keeps that user-oriented idea while recasting it for modern C\# APIs with exceptions, status-bearing results, residual diagnostics and automated tests.
...
\appendix
\chapter{Historical context and V1 coverage}
The C\# edition deliberately follows the major numerical problem classes of the historical Numerical Methods Toolbox at a high level. It is not a source-code port. Public APIs, data types, error handling, examples and diagnostics belong to SASD Math Toolkit.

\begin{longtable}{@{}p{0.28\textwidth}p{0.62\textwidth}@{}}
\toprule
\textbf{Historical topic} & \textbf{C\#/.NET edition}\\
\midrule
...
Matrix routines & \texttt{DenseMatrix}, Gaussian elimination, pivoted LU, Householder QR, inverse, residuals and Gauss-Seidel\\
...
Least squares & Polynomial/general-basis fits and named models, modernized to Householder QR\\
FFT / applications & Complex and real FFT, compact real spectrum, convolution and cross-correlation\\
Graphics demonstrations & A platform-neutral deterministic HTML/SVG sample instead of coupling numerics to a GUI framework\\
\bottomrule
\end{longtable}
...
SASD Math Toolkit is licensed under MIT. The historical Borland manual and historical product names remain the property of their respective rights holders. This handbook documents the independently developed SASD C\#/.NET implementation and does not reproduce the historical manual text.
