...
Der V1-Bereich der Nullstellensuche ist vollständig abgedeckt. Auch direkte LU-Faktorisierung, Eigenwertbereich und Differentiation sind vollständig. Die RK4-Komfortfamilie im ODE-Bereich deckt skalare Gleichungen erster, zweiter und n-ter Ordnung sowie gekoppelte Systeme erster und zweiter Ordnung ab und verwendet dabei denselben gemeinsamen System-RK4-Kern. Zusätzlich stehen adaptives skalares RKF45 und der Adams-Bashforth-/Adams-Moulton-Prädiktor-Korrektor vierter Ordnung zur Verfügung. Der historische V1-Randwertbereich deckt lineares und nichtlineares RK4-gestütztes Shooting für skalare Dirichlet-Probleme zweiter Ordnung ab.

Im Least-Squares-Bereich waren Polynomfits und beliebige lineare Basisfunktionen bereits vorhanden. Die benannten historischen Modellhelfer werden nun schrittweise ergänzt; begonnen wurde mit einer Potenzgesetz-Anpassung für positive Daten einschließlich eigenem Ergebnismodell und Residualdiagnose im ursprünglichen Wertebereich.

Unter `docs/user-guide/` wächst parallel ein eigenes C#/.NET-Benutzerhandbuch. Mit den benannten Regressionsmodellen wurde nun auch das Least-Squares-Kapitel begonnen.
...
Alle noch offenen Punkte der englischen Kompatibilitätsmatrix schließen: insbesondere die verbleibenden benannten Least-Squares-Modelle, FFT-Hilfsfunktionen und Demo-/Beispielanwendungen. Zusätzlich müssen verbleibende mit `partial` gekennzeichnete Punkte wie die fünfgliedrige Fourier-Komfort-API und die kompakte Real-Spektrum-FFT-API abgeschlossen oder bewusst als durch allgemeinere APIs ersetzt dokumentiert werden.
