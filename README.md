...
Modernization is now in progress. The modern dense numerical foundation includes **Householder QR**, **Cholesky**, a **one-sided Jacobi singular value decomposition**, stable matrix norms and shared rank/nullity/conditioning diagnostics. General/polynomial least squares no longer rely on normal equations; the general basis API can use SVD for rank-deficient or underdetermined minimum-norm problems. Sparse linear algebra now includes an immutable canonical **CSR** matrix, sparse arithmetic/norms and the first iterative Krylov solver: **Conjugate Gradient for SPD systems**, with shared residual/tolerance diagnostics and true-residual verification. The broader plan is documented in [`docs/en/MODERNIZATION-2026.md`](docs/en/MODERNIZATION-2026.md).
...
- Sparse linear algebra: immutable canonical **CSR** storage, coordinate/dense/raw-CSR construction, duplicate aggregation, sparse matrix-vector multiplication with reusable span buffers, transpose/dense conversion, sparse 1-/infinity-/Frobenius norms, structural symmetry diagnostics and **Conjugate Gradient for SPD systems** with explicit iteration/residual diagnostics
...
- Sparse Conjugate Gradient: [`docs/en/SPARSE-CONJUGATE-GRADIENT.md`](docs/en/SPARSE-CONJUGATE-GRADIENT.md)
